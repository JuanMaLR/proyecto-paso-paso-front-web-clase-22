//Hacer el import de useState, useEffect y useRef
import React, { useState, useEffect, useRef } from 'react';

function SearchMovies(){
	const [movies, setMovies] = useState([]);
	const [keyword, setKeyword] = useState('Hulk');
	const pelicula = useRef(null);

	// Colocar aquí tu apiKey
	...
	const buscarPelicula = async (e) => {
		...
		setKeyword(pelicula.current.value);

		pelicula.current.value = "";
	}
						...
						<div className="col-12 col-md-6">
							...
							<form method="GET" onSubmit={buscarPelicula}>
								<div className="form-group">
									...
									<input ref={pelicula} type="text" className="form-control" />
								</div>
